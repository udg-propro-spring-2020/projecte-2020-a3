...
public class ToJSONParserHelper {
    ...
    public static String saveChessConfigToJSON(Chess chess) throws NullPointerException {
        ...
        s.append(OBJ_START).append(propertyToJSON("nFiles", chess.rows(), false, true, "\t"))
                ...
                .append(objectListToJSON("enrocs", chess.castlings(), true, "\t")).append(OBJ_END);
    ...
    /// it
    /// @throws IllegalArgumentException If @param identation does no contain spaces
    /// or tabs
    ...
    /// @throw IllegalArgumentException If @param identation does not contain only
    /// spaces or tabs
    ...
    /// @throw NullPointerException If the @param list is null or contains a null
    /// item
    /// @thwo IllegalArgumentException If the @param identation does not contain
    /// only spaces or tabs
    ...
    /// @throw NullPointerException If the @param list is null or contains a null
    /// item
    /// @throw IllegalArgumentException If the @param identation does not contain
    /// only spaces or tabs
        ...
        if (list.isEmpty()) {
            ...
            for (int i = 0; i < list.size() - 1; i++) {
                ...
                s.append(identation + ONE_TAB).append(OBJ_START).append(item.first.toJSON())
                        .append(item.second.toJSON()).append(NEXT_LINE).append(identation + ONE_TAB).append(OBJ_END)
                        .append(EOL);
            }
            ...
            s.append(identation + ONE_TAB).append(OBJ_START).append(item.first.toJSON()).append(item.second.toJSON())
                    .append(NEXT_LINE).append(identation + ONE_TAB).append(OBJ_END).append(identation).append(LIST_END);
        }
