...
public class Chess {
    ...
    del tauler inicial ordenadoes.
    Al for de mes enbaix s'omplen les dues primeres files i les dues ultimes, les demes
    queden a null.
    També s'omplen les array de peces vives de cada jugador.
    */
		...
		Piece p = board[x0][y0];
                ...
                while(i < pieceMovements.size() && !found){
                    ...
                    if(y1-y0 == act.movY() && x1-x0 == act.movX()){//fer-ho per les dues parts del tauler                        
                        ...
                        }else if(!enemiePieceOnDestiny && act.captureSign() != 2){//Si no hi ha peça i no es un moviment que nomes fa per matar
                            r.first = true;
                            r.second = null; 
                            found = true;
                        ...
                        //else if(y1-y0 == pieceMovements.get(i).MovY() && x1-x0 == pieceMovements.get(i).MovX() && pieceMovements.get(i).)
